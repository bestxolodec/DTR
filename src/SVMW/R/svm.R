...
svm.formula <- function(formula, 
  ...
  if( is(w,"vector") ) w <- t(t(w))
  if( !missing(subset) ) w <- w[subset,]
  ...
  ret <- svm.default (x, y, scale = scale, ..., w, na.action = na.action)
...
svm.default <- function (x,
  ...
  if(inherits(x, "Matrix")) {
    if( !requireNamespace("SparseM", quietly=TRUE) ) {
      stop("svmw requires the SparseM package available on CRAN.")
    }
    if( !requireNamespace("Matrix", quietly=TRUE) ) {
      stop("svmw requires the Matrix package available on CRAN.")
    }
    x <- as(x, "matrix.csr")
  }
  ...
  if(is.null(w)) stop(sQuote("w"), " must not be NULL!")
  ...
  if( cret$error != empty_string ) stop(paste(cret$error, "!", sep=""))
...
predict.svm <- function(object, 
  ...
  if( object$tot.nSV < 1 ) stop("Model is empty!")
  ...
  act <- NULL
  if( (is.vector(newdata) && is.atomic(newdata)) ) newdata <- t(t(newdata))

  if( sparse ) newdata <- SparseM::t(SparseM::t(newdata))
  ...
  if( !is.null(act) && !preprocessed ) rowns <- rowns[-act]

  if( any(object$scaled) ) {
    newdata[,object$scaled] <- scale(newdata[,object$scaled, drop = FALSE],
  ...
  if( is.character(object$levels) ) { # classification: return factors
    ret2 <- factor (object$levels[ret$ret], levels = object$levels)
    ...
    if( fill ) {
      ...
      for( i in names(which(sapply(data, is.factor))) ) {
        if (!is.factor(slice[[i]])) {
          ...
          if( is.na(fac) ) {
            stop(paste("Level", dQuote(lev), 
                       "could not be found in factor", sQuote(i)))
          }
          slice[[i]] <- fac
        }
      }
