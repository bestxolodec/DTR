/* from e1071 with tweaks for weights */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <R.h>
			 ...
			 double* ctotal2)
{
	...
	for(i=0; i<prob->l; i++)
	{
		...
		tw = prob->W[i];  /*STH ADDED*/
		prob->W[i] = prob->W[j];  /*STH ADDED*/
		prob->W[j] = tw;  /*STH ADDED*/
	}
	PutRNGstate();
	...
	for(i=0; i<nr_fold; i++)
	{
		...
		subprob.W = Malloc(double,subprob.l);  /*STH ADDED*/
		...
		for(j = 0; j < begin; j++)
		{
			...
			subprob.W[k] = prob->W[j];  /*STH ADDED*/
			++k;
		}
	       ...
	       char   **error)
{
    ...
    /* looks like we may need to add -v n, -q, and -W */
